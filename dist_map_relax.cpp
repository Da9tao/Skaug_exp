...
double compute_dt(double t_scale, double D0_ref) {
    ...
    const double relaxation_time = (R_ref_um * R_ref_um) / D0_ref / 10.0;
    const double dt_limit = relaxation_time * t_scale;

    if (dt_limit > 0.0) {
        const double safe_limit = 0.9 * dt_limit;
        return min(0.0001, safe_limit);
    }

    return 1e-8;
}

void validate_compute_dt_bounds() {
    const double t_scale = 1.0;
    const double extreme_diffusivity = 1e8;
    const double R_ref_um = 0.001 * R_ref;
    const double relaxation_time = (R_ref_um * R_ref_um) / extreme_diffusivity / 10.0;
    const double dt_limit = relaxation_time * t_scale;
    const double dt = compute_dt(t_scale, extreme_diffusivity);

    if (!(dt_limit > 0.0 && dt > 0.0 && dt < dt_limit)) {
        throw runtime_error("compute_dt does not respect relaxation bound for extreme diffusivity");
    }
}
